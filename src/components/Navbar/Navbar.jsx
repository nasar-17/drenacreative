...
const NAV_ITEMS = [
  ...
  { label: 'Produk Kami', to: '/produk' },
  {
    label: 'Layanan Website',
    to: '/layanan-website',
    children: [
      { label: 'Landing Page', hash: 'landing-page' },
      { label: 'Company Profile', hash: 'company-profile' },
      { label: 'Website UMKM', hash: 'website-umkm' },
      { label: 'Website Portfolio', hash: 'website-portfolio' },
      { label: 'Website Custom', hash: 'website-custom' },
    ],
  },
  {
    label: 'Layanan AI Content Branding',
    to: '/layanan-ai-content-branding',
    children: [
      { label: 'AI Copywriting & Strategy', hash: 'ai-copywriting' },
      { label: 'Social Media AI Content', hash: 'social-media-generation' },
      { label: 'AI Visual & Imagery', hash: 'ai-visual-generation' },
      { label: 'Brand Voice Model Setup', hash: 'brand-voice-setup' },
    ],
  },
  {
    ...
    to: '/layanan-aplikasi',
    children: [
      { label: 'Web Application', hash: 'web-app' },
      { label: 'Mobile Application', hash: 'mobile-app' },
      { label: 'SaaS MVP Development', hash: 'saas-mvp' },
      { label: 'API & Backend System', hash: 'api-backend' },
    ],
  },
];
...
/* ══════════════════════════════════════════════════════════════════════════
   DESKTOP DROPDOWN
   ══════════════════════════════════════════════════════════════════════════ */
function DesktopDropdown({ item, scrolled, onItemClick }) {
  return (
    <motion.div
      ...
      <div className="bg-white dark:bg-primary-900 backdrop-blur-md rounded-xl shadow-[0_12px_40px_rgba(53,88,114,0.12)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.4)] border border-neutral-100 dark:border-white/10 overflow-hidden">
        {item.children.map((child) => (
          <Link
            ...
            onClick={() => onItemClick(item.to, child.hash)}
...
export function Navbar() {
  ...
  /* ── Dropdown item click ── */
  const handleDropdownItemClick = (parentTo, hash) => {
    setActiveDropdown(null);
    setMenuOpen(false);
    setMobileOpenDropdown(null);
    if (location.pathname === parentTo) {
    ...
    // If different page, Link navigation + ScrollToTop handles hash scroll
  };

  /* ── Scroll to #kontak on current page ── */
  ...
  const navTextBase = (isInMenu) => {
    ...
    if (scrolled) return 'text-neutral-500 dark:text-neutral-300 hover:text-primary dark:hover:text-white';
        ...
        >
              ...
              {item.children ? (
                ...
                >
                  <NavLink
                    to={item.to}
                    className={({ isActive }) =>
                      `flex items-center gap-1 font-body font-medium transition-colors relative group text-sm tracking-[0.04em] ${
                        isActive
                          ? 'text-primary dark:text-tertiary-300'
                          : scrolled
                      ...
                      <>
                        ...
                        <span
                          className={`absolute -bottom-1 left-0 h-[1.5px] transition-all duration-300 ${
                            isActive ? 'w-full bg-primary dark:bg-tertiary-300' : 'w-0 group-hover:w-full ' + underlineColor(false)
                          }`}
                        />
                      </>
                    )}
                ...
                <div className="hidden md:block">
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `font-body font-medium transition-colors relative group text-sm tracking-[0.04em] ${
                        isActive
                          ? 'text-primary dark:text-tertiary-300'
                          : scrolled
                          ? 'text-neutral-500 dark:text-neutral-300 hover:text-primary dark:hover:text-white'
                ...
                <div className="md:hidden w-full">
                  ...
                  >
                    <NavLink
                      ...
                      onClick={(e) => {
                        // Let the navigate happen but also toggle accordion
                        setMobileOpenDropdown((prev) => (prev === item.label ? null : item.label));
                      }}
                    >
                      {item.label}
                        ...
                        <ul className="flex flex-col pl-4 pt-2 pb-1 gap-3 border-l border-white/15 ml-1 mt-1">
                          {item.children.map((child) => (
                            <li key={child.hash}>
                              <Link
                                to={`${item.to}#${child.hash}`}
                                onClick={() => handleDropdownItemClick(item.to, child.hash)}
          ...
          <li className="w-full md:w-auto">
            ...
            <a
              ...
              className={`hidden md:inline-block font-body font-medium transition-colors relative group text-sm tracking-[0.04em] ${navTextBase(false)}`}
            >
              Kontak
              <span className={`absolute -bottom-1 left-0 h-[1.5px] w-0 transition-all duration-300 group-hover:w-full ${underlineColor(false)}`} />
            </a>
            {/* Mobile */}
            <a
              href="#kontak"
